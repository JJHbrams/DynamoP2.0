...
void init_ODE(){
  ...
  dBodySetPosition (body[0],start[0],start[1],STARTZ);
  ...
  dBodySetPosition (body[1],0.5*LENGTH+start[0],start[1],STARTZ-HEIGHT*0.5);
  dBodySetPosition (body[2],-0.5*LENGTH+start[0], WIDTH*0.5+start[1],STARTZ-HEIGHT*0.5);
  dBodySetPosition (body[3],-0.5*LENGTH+start[0],-WIDTH*0.5+start[1],STARTZ-HEIGHT*0.5);
  double roll=0. ,pitch=0. ,yaw=start[2];
  dMatrix3 R;
...
void prop_cb(const dynamo_planner::custom_states_msgs& _msg){
  ...
  double roll=0. ,pitch=0. ,yaw=_msg.yaw, delta;
  dMatrix3 R;
  ...
  const double* const dquar2 = dBodyGetQuaternion(body[1]);//Steer angle
  ...
  tf2::Quaternion quat(dquar[0],dquar[1],dquar[2],dquar[3]);
  tf2::Matrix3x3 mat(quat);
  mat.getRPY(roll,pitch,yaw);
  // std::cout << "Get position : " << dpos[0] << ", " << dpos[1] << ", " << dpos[2] << std::endl;
  tf2::Quaternion quat2(dquar[0],dquar[1],dquar[2],dquar[3]);
  tf2::Matrix3x3 mat2(quat);
  mat2.getRPY(roll,pitch,steer);
  // Pose
  posX = dpos[0];
  posY = dpos[1];
  posYAW = yaw;
