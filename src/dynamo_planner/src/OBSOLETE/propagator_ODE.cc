...
namespace gazebo
{
	class propagator : public ModelPlugin
	{
		...
		private: dSpaceID dspace;
		private: dBodyID body[4];
		private: dJointID joint[16];	// joint[0] is the front wheel
		private: static dJointGroupID contactgroup;
		private: dSpaceID car_space;
		private: dGeomID box[1];
		private: dGeomID sphere[3];
		...
		public: propagator(){
			ROS_INFO("Initailize Physics engine - ODE");
		  ...
		  dBodySetPosition(body[0],0.0,0.0,STARTZ);
		...
		public: void Load(physics::ModelPtr _parent, sdf::ElementPtr)//Initialization
		{
			...
			//TEST simloop
			// dsSimulationLoop(1,NULL,352,288,&fn);
		...
		public: void P2G_cb(const dynamo_planner::custom_states_msgs& _msg){
			...
			std::cout <<"Get pose : " << dpos[0] << ", " << dpos[1] << ", " << yaw << std::endl;
		  ...
		  // dBodySetPosition(body[3],-0.5*LENGTH+_msg.x,-WIDTH*0.5+_msg.y,STARTZ-HEIGHT*0.5);
			...
			// std::cout << "Get pose : " << dpos[0] << ", " << dpos[1] << ", " << yaw << std::endl;
			// std::cout << "Get rotation : " << yaw/180*M_PI << "degrees" << std::endl;
	...
	// double propagator::ctrl, propagator::ctrlYAW;
	// double propagator::time;
