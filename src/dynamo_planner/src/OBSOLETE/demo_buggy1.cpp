...
void vehicle_def(dReal X_POS, dReal Y_POS, dReal YAW, dReal DELTA){
  ...
  for (i=0; i<=3; i++){
    POS[i][X] = (ORI[i][X]+X_POS)*cos(YAW)-(ORI[i][Y]+Y_POS)*sin(YAW);
    POS[i][Y] = (ORI[i][X]+X_POS)*sin(YAW)+(ORI[i][Y]+Y_POS)*cos(YAW);
    POS[i][Z] = ORI[i][Z];
  }
  ...
  dRFromEulerAngles(R,M_PI/2,YAW+DELTA,0);
  ...
  dJointSetHinge2Axis2(joint[0],MV_AX(YAW+DELTA),MV_AY(YAW+DELTA),0);
...
void init_ODE(){
  ...
  ORI[1][X] = 0.5*LENGTH;   ORI[1][Y] = 0.0;         ORI[1][Z] = STARTZ-HEIGHT*0.5;
  ORI[2][X] = -0.5*LENGTH;  ORI[2][Y] = WIDTH*0.5;   ORI[2][Z] = STARTZ-HEIGHT*0.5;
  ORI[3][X] = -0.5*LENGTH;  ORI[3][Y] = -WIDTH*0.5;  ORI[3][Z] = STARTZ-HEIGHT*0.5;
  ...
  vehicle_def(X_init,Y_init,YAW_init,DELTA_init);
...
void prop_cb(const dynamo_planner::custom_states_msgs& _msg){
  ...
  PropStates.control = _msg.control;
  PropStates.controlYAW = _msg.controlYAW;
  time = _msg.duration;
  // ctrl = sqrt(pow(_msg.controlX,2)+pow(_msg.controlY,2));
  ctrl = PropStates.control;
  ctrlYAW = PropStates.controlYAW;
  ...
  /********************* Physics engine RUN *****************/
  // ODE buggy
  vehicle_def(_msg.x, _msg.y, _msg.yaw, steer);
  ...
  std::cout << "A.Get pose : X=" << dpos[0] << ", Y=" << dpos[1] << ", YAW=" << yaw/M_PI*180 << ", STEER=" << steer/M_PI*180 << std::endl;

  for(int cnt=0;cnt<1000;cnt++) {
    // motor
    dJointSetHinge2Param (joint[0],dParamVel2,-ctrl);
    dJointSetHinge2Param (joint[0],dParamFMax2,0.1);
    // steering
    // dReal wd = ctrlYAW;
    dJointSetHinge2Param (joint[0],dParamVel,ctrlYAW);
    dJointSetHinge2Param (joint[0],dParamFMax,0.2);
    dJointSetHinge2Param (joint[0],dParamLoStop,steer-0.75);// 0.75 : 45 degrees
    dJointSetHinge2Param (joint[0],dParamHiStop,steer+0.75);
    dJointSetHinge2Param (joint[0],dParamFudgeFactor,0.1);

    dSpaceCollide (space,0,&nearCallback);
    dWorldStep (world,time/1000);//1ms
  ...
  posY = dpos[1];
  posYAW = yaw;
  std::cout << "B.Get pose : X=" << dpos[0] << ", Y=" << dpos[1] << ", YAW=" << yaw/M_PI*180 << ", STEER=" << steer/M_PI*180 << std::endl;
  /********************* Physics engine END *****************/
  // Update States
  PropStates.x = posX;
  PropStates.y = posY;
...
int main (int argc, char **argv)
{
  ...
  */
  ...
  //Publisher
  ode2planner	= nh.advertise<dynamo_planner::custom_states_msgs>("/ODE_States", 1, true);
  //Subscriber
