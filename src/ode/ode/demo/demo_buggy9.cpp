...
std::fstream dataout(TE_FILE, std::ios::out);
...
*/
  ...
  double NUM_TE = 5000;
  if(NUM_SAMPLES >= NUM_TE) {
    dataout.close();
    return;
  }

  if(RESET_cnt == 5) {
    X_set = RAND_VAL(-20,20,'u'), Y_set = RAND_VAL(-20,20,'u'), YAW_set = RAND_VAL(-M_PI,M_PI,'u');
...
*/
  ...
  for(int cnt=0;cnt<TIME_set;cnt++) {
...
*/
  ...
  std::cout.width(12);std::cout << dyaw2/M_PI*180 << std::endl;
