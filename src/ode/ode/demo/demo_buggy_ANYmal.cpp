...
/*
  ...
  // Body-hip joints
  for (i=0; i<3; i++) {//Main body-belly
    dJointAttach(joint[i],body[0],body[i+1]);
    dJointSetFixed(joint[i]);
  }
  for (i=3; i<7; i++) {//Main body-HAA joints(HAA actuator)
    dJointAttach(joint[i],body[0],body[i+1]);
    const dReal *axis = dBodyGetPosition(body[i+1]);
    dJointSetHingeAnchor(joint[i],axis[0],axis[1],axis[2]);
    dJointSetHingeAxis(joint[i],1,0,0);
    dJointSetHingeParam (joint[i],dParamLoStop,-M_PI/12);
    dJointSetHingeParam (joint[i],dParamHiStop,M_PI/12);
  }
  ...
  for (i=15; i<19; i++) {//HFE joints(protector-thigh)
    ...
    dJointSetHingeParam (joint[i],dParamLoStop,-M_PI/8);
    dJointSetHingeParam (joint[i],dParamHiStop,M_PI/8);
  }
...
static void simLoop (int pause){
  ...
  dReal T_HAA_A = RAND_VAL(-100.0,100.0,'n'), T_HAA_B = RAND_VAL(-100.0,100.0,'n'), T_HAA_C = RAND_VAL(-100.0,100.0,'n'), T_HAA_D = RAND_VAL(-100.0,100.0,'n');
  dReal T_HFE_A = RAND_VAL(-100.0,100.0,'n'), T_HFE_B = RAND_VAL(-100.0,100.0,'n'), T_HFE_C = RAND_VAL(-100.0,100.0,'n'), T_HFE_D = RAND_VAL(-100.0,100.0,'n');
  dReal T_KFE_A = RAND_VAL(-100.0,100.0,'n'), T_KFE_B = RAND_VAL(-100.0,100.0,'n'), T_KFE_C = RAND_VAL(-100.0,100.0,'n'), T_KFE_D = RAND_VAL(-100.0,100.0,'n');
  dReal TIME_set = RAND_VAL(100, 5000,'u');

  double SIM_TIME = 0.0001;//0.581393388217187;
  double TIME_DENOM = 30;
...
*/
  ...
  // dJointSetHingeParam (joint[15],dParamFMax,50);
  for(int cnt=0;cnt<TIME_DENOM;cnt++) {
    // Control
    // dJointAddHingeTorque (joint[3],T_HAA_A);dJointAddHingeTorque (joint[4],T_HAA_B);dJointAddHingeTorque (joint[5],T_HAA_C);dJointAddHingeTorque (joint[6],T_HAA_D);
    dJointAddHingeTorque (joint[15],T_HFE_A);dJointAddHingeTorque (joint[16],T_HFE_B);dJointAddHingeTorque (joint[17],T_HFE_C);dJointAddHingeTorque (joint[18],T_HFE_D);
    dJointAddHingeTorque (joint[23],T_KFE_A);dJointAddHingeTorque (joint[24],T_KFE_B);dJointAddHingeTorque (joint[25],T_KFE_C);dJointAddHingeTorque (joint[26],T_KFE_D);
...
void init_ODE(){
  ...
  for (i=3; i<7; i++) joint[i] = dJointCreateHinge(world,0);//HAA
  for (i=7; i<11; i++) joint[i] = dJointCreateFixed(world,0);//HAA-protector
  for (i=11; i<15; i++) joint[i] = dJointCreateFixed(world,0);//protector-heatfins
  for (i=15; i<19; i++) joint[i] = dJointCreateHinge(world,0);//HFE
  for (i=19; i<23; i++) joint[i] = dJointCreateFixed(world,0);//thigh-upper protector
  for (i=23; i<27; i++) joint[i] = dJointCreateHinge(world,0);//KFE
