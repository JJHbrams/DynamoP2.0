...
static void nearCallback (void *, dGeomID o1, dGeomID o2){
  ...
  if (num_Col > 0) {
    for (i=0; i<num_Col; i++) {
      ...
      dJointAttach (col, dGeomGetBody(contact[i].geom.g1),
...
void ANYmal_def(){
  ...
  // dMassSetParameters(&mass, MASS, -0.1, -0.1, -0.1,ixx, iyy, izz, ixy, ixz, iyz);
...
void init_ODE(){
  ...
  for (i=3; i<7; i++) joint[i] = dJointCreateHinge2(world,0);//HAA
