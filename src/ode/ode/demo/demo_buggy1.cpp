...
dsFunctions fn;
static dReal speed=10,steer=-M_PI/4;	// user commands
...
*/
  vehicle_def(X_set,Y_set,M_PI/6,DELTA);
  ...
  for(int cnt=0;cnt<1000;cnt++) {
    ...
    if (!pause) {
      ...
      // steering
      /*dReal wd = steer - dJointGetHinge2Angle1(joint[0]) ;
      if (wd > 0.1) wd = 0.1;// Maximum steer velcity(wd) : 0.1rad/step
      if (wd < -0.1) wd = -0.1;
      wd *= 10.0;*/
      dReal wd = steer;
      dJointSetHinge2Param (joint[0],dParamVel,wd);
      dJointSetHinge2Param (joint[0],dParamFMax,0.2);
      dJointSetHinge2Param (joint[0],dParamLoStop,DELTA-0.75);// 0.75 : Around 45 dgrees
      dJointSetHinge2Param (joint[0],dParamHiStop,DELTA+0.75);
...
int main (int argc, char **argv)
{
  ...
  while(1){
    ...
    for(int cnt=0;cnt<1000;cnt++) {
      ...
      // steering
      dJointSetHinge2Param (joint[0],dParamVel,steer);
      dJointSetHinge2Param (joint[0],dParamFMax,0.2);
